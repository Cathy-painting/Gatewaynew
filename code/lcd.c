...
{
	uint16_t temp = GPIOC->ODR;//	执行前先把pc端的输出状态全部存到这个变量里
	u32 i = 0;
	...
	/*
	 * BUG FIX: LCD_DrawChar 循环结束后，LCD_WriteReg 会留下 CS=0（LCD 仍选中）。
	 * 此时如果直接恢复 GPIOC->ODR（LED 状态），LCD 会把 LED 数据当作命令解析，
	 * 导致控制器状态混乱 → 白屏。
	 *
	 * 修复：恢复 GPIOC->ODR 前，先将 CS 拉高（取消选中 LCD）。
	 */
	GPIOB->BSRR |= GPIO_PIN_9;   /* CS = 1, 取消选中 LCD */
   GPIOC->ODR = temp;
}
/*******************************************************************************
